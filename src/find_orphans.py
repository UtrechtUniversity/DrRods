...
import icat
import pathdb
from drrods_common import localhost_name, load_dict, dump_dict, \
    SIGNATURE_REPLICA_DATA, SIGNATURE_ORPHAN_FILES, SIGNATURE_ORPHAN_REPLICAS
import drrods_sql
import argparse
...
def retrieve_icat_data_from_database(connection, host):
    ...
    (db['replica_count'], db['replica_data_paths']) = \
        drrods_sql.replica_paths_for_host(connection, host)
...
def main(cmd):
    if cmd._import and cmd._host:
        print('''
Warning: Incompatible argument (hostname): Hostname will be derived
from import file''')
    ...
    if cmd._import:
        ...
        print('Retrieving data on host {} from ICAT database'
              .format(target_host))
    ...
    if db['host'] == localhost_name():
        ...
        print('Using ICAT data that pertains to another host ({}).'
              .format(db['host']))
...
'''.format(vault_count, replica_count, data_path_count))
    ...
    # (4) compare ICAT data with actual data files to locate
    #     any discrepancies (orphans)
    if db['host'] != localhost:
        print('''
Skipping scan for orphans/missing files because the data pertains
to another host''')
        sys.exit(0)

    # (4a) find data files in vaults that are not registered in
    #      the ICAT (orphan data files)
    print('''
Scanning iRODS resource vault dirs for orphan files...(this may take awhile)''')
    count = 0
    ...
    for vault_dir in db['vaults']:
        ...
        find_orphan_data_files(
            db['replica_data_paths'], vault_dir, orphan_files)
        orphans_in_vault = orphan_files.size() - count
        count = count + orphans_in_vault
        print('{} orphan files in vault {}'
              .format(orphans_in_vault, vault_dir))

    print('\n{} orphans in total in above vaults'.format(count))

    # (4b) find replicas that reference a non-existing data file
    #      (orphan replicas)
    print('''
\nChecking for data files referenced by replicas, yet missing in vault:''')
    ...
    # (5) store output data as dict file (suitable for processing
    #     by compagnion tools)
    if cmd.summary:
    ...
    if orphan_files.size() > 0:
        ...
        dump_dict("orphan_files_" + db['host'] + ".bin", orphans,
                  SIGNATURE_ORPHAN_FILES)
    ...
    if orphan_replicas.size() > 0:
        ...
        dump_dict("orphan_replicas_" + db['host'] + ".bin",
                  orphans, SIGNATURE_ORPHAN_REPLICAS)
        print("Data on orphan replicas written to file: "
              + path_orphan_replicas)
...
orphan_files_<hostname>.bin    - files in vault yet not registered as a replica
orphan_replicas_<hostname>.bin - replicas that reference non-existing data file

The output files are in a binary format, suitable for processing
by other DrRods tools
''')
    parser.add_argument(
        '-i', '--import', nargs=1, metavar='<filename>', dest='_import',
        help='''
import ICAT data from a file (instead of retrieving from the database)''')
    parser.add_argument(
        '-e', '--export', nargs=1, metavar='<filename>', dest='_export',
        help='export retrieved/imported ICAT data to a file')
    parser.add_argument(
        '-H', '--host', nargs=1, metavar='<hostname>', dest='_host',
        help='''
use data pertaining to this host (fully-qualified domain name)''')
    parser.add_argument(
        '-s', '-summary', action='store_true', dest='summary',
        help='''
only provide summary of findings, do not generate output data files''')
