...
def main(args):
    ...
    if not db:
        sys.exit(1)
    print_stderr(
        "{} orphan data files for host {} have been loaded from the inputfile"
        .format(db['file_paths'].size(), db['host']))
...
#/bin/bash\n#generated by DrRods tooling\n# Disclaimer: Use at own risk\n''')
        ...
        if args.dryrun:
            f.write('''
echo "THIS IS A *DRYRUN*, SEE BELOW THE LIST OF FILES THAT WOULD BE DELETED"'''
                    + "\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='''
Prepares rm commands for data files that are not referenced by any replica''')
    parser.add_argument(
