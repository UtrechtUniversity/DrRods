...
import pathdb
from drrods_common import print_stderr
...
def inconsistent_paths_columns():
    return [
        ...
        'data_checksum', 'modify_ts', 'resc_name', 'actual_data_path',
        'expected_data_path',
...
from (
   select
      ...
      substr(d.data_path,
         length(d.data_path) - position('/' in reverse(d.data_path)) + 2 )
         as BASENAME
   from r_data_main d, r_coll_main c, r_resc_main r
   where
      d.resc_id = r.resc_id and d.coll_id = c.coll_id
      -- filter out s3 and other resources where directory and
      -- coll_name are not kept in sync
...
def count_other_replicas_with_same_data_path(
        icat_connection, data_id, data_repl_num, data_path):
    query = """
    ...
    try:
        with icat_connection.cursor() as cur:
            cur.execute(query, (data_id, data_repl_num, data_path,
                                data_id, data_repl_num))
            row = cur.fetchone()
            replica_count = row[0]
    except (Exception) as error:
        print_stderr(error)
            ...
            for row in icat_connection.iter_row(cur):
                if data_id != row[0]:
                    ...
                    if len(problems) > 0:
                        ...
                        data_objects.append(
                            (data_id, ref[3], ref[2], list(problems.keys())))
                        problems = {}
